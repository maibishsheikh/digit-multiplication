...
export default function BossBattle({ questions, world, worldAccent, onComplete, playNarration }) {
  ...
  const boss = world?.boss ?? { name: 'Ratio Boss', emoji: '👑', reward: 'Ratio Badge 👑' };
    ...
    return (
      <div className="boss-banner glass-card">
        ...
        <div className="conversion-fact-box" style={{ marginBottom: 20 }}>
          🔑 Golden Rule: <span className="mass-value">Divide both terms by GCF</span>
        </div>
  ...
  if (won) {
    return (
      ...
      >
        ...
        <p style={{ color: 'rgba(255,255,255,0.85)', marginBottom: 8, fontSize: '1.1rem' }}>
  ...
  if (lost) {
    return (
      <div className="boss-result-screen">
        <div className="boss-result-emoji">💔</div>
        <h3 style={{ fontFamily: 'var(--font-display)', color: 'var(--red)', marginBottom: 8, fontSize: '1.4rem' }}>
  ...
  return (
    <div style={{ position: 'relative', width: '100%' }}>
      ...
      <FeedbackOverlay
        ...
        message={selected === question?.correctAnswer ? "That's exactly right! Well done! 🎉" : "Not quite! Remember to divide both terms by GCF."}
