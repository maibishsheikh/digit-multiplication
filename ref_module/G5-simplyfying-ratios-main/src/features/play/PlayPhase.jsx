...
function WorldSelector({ onSelect, worldProgress }) {
  return (
    <div className="play-screen">
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.6rem', textAlign: 'center', flexShrink: 0 }}>
        ...
        {WORLDS.map(world => {
          ...
          return (
            <motion.div key={world.id} className="world-card glass-card"
              ...
              <div className="world-emoji" style={{ '--icon-glow': `${world.accent}38`, borderColor: `${world.accent}55` }}>{world.emoji}</div>
...
function ModeSelector({ world, onSelect, onBack }) {
  return (
    <div className="play-screen">
      <button onClick={onBack} style={{ background: 'none', border: 'none', color: 'var(--color-text-muted)', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 700, alignSelf: 'flex-start', flexShrink: 0, fontSize: '1rem' }}>← Back to Worlds</button>
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.5rem', textAlign: 'center', flexShrink: 0 }}>
        {world.emoji} {world.name} — Practice Modes
      </h2>
  ...
  return (
    <div className="play-screen">
      <motion.div className="glass-card"
        style={{ maxWidth: 460, width: '100%', padding: '28px 32px', textAlign: 'center' }}
        initial={{ scale: 0.88, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
        <div style={{ fontSize: '3rem', marginBottom: 6 }}>{world.emoji}</div>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.7rem', color: 'var(--gold)', marginBottom: 6 }}>
          {world.name} Complete!
        </h2>
        <StarRow stars={stars} />
        <p style={{ margin: '12px 0', fontSize: '1.1rem' }}>
          {result.correct} / {result.total} correct · {result.xp} XP
        </p>
        {result.boss && !result.lost && (
          <p style={{ color: 'var(--gold)', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.1rem', marginBottom: 6 }}>
    ...
    <AnimatePresence mode="wait">
      ...
      {screen === 'playing' && (
        <motion.div key="playing" style={{ position: 'absolute', inset: 0, top: 64, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 14px', overflowY: 'auto', overflowX: 'hidden' }}
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, width: '100%', maxWidth: 640, flexShrink: 0, fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.9rem', color: 'var(--color-text-muted)' }}>
