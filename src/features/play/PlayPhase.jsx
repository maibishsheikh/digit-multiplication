...
function WorldSelector({ onSelect, worldProgress }) {
  return (
    <div className="play-screen play-screen--scroll">
...
export default function PlayPhase({ onComplete, playNarration }) {
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      {screen === 'playing' && (
        <motion.div key="playing" style={{ position: 'absolute', inset: 0, top: 84, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 14px', overflowY: 'auto', overflowX: 'hidden' }}
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          {/* Breadcrumb */}
