...
const TAKEAWAYS = [
  { icon: '🧩', text: 'Break a number apart by place value, then multiply each part — this is the area model.' },
  { icon: '🔢', text: 'The standard algorithm multiplies each digit and regroups (carries) into the next place.' },
  { icon: '➡️', text: 'For 3-digit × 2-digit, shift the second partial product one place left before adding.' },
  { icon: '✔️', text: 'Estimate first by rounding, then check your exact answer makes sense.' },
];

const CONFIDENCE_OPTIONS = [
  { emoji: '🌟', text: "I'm great at multiplying big numbers!" },
  { emoji: '👍', text: 'I can multiply most of the time!' },
    ...
    <div>
      ...
      <div className="options-grid">
        {question.options.map(opt => {
          ...
          return <button key={opt} className={cls} onClick={() => pick(opt)} style={{ textAlign: 'center' }}>{typeof opt === 'number' ? opt.toLocaleString() : opt}</button>;
        })}
      </div>
    </div>
  );
    ...
    return (
      <div className="reflect-screen" style={{ justifyContent: 'center' }}>
        <div className="reflect-card glass-card" style={{ width: '100%', maxWidth: 520, textAlign: 'center' }}>
          ...
          <p style={{ fontWeight: 700, marginBottom: 16 }}>How confident do you feel about multiplying big numbers?</p>
    ...
    <div className="reflect-screen">
      <div style={{ width: '100%', maxWidth: 560, display: 'flex', flexDirection: 'column', gap: 14, paddingBottom: 16 }}>
        ...
        <div className="conversion-fact-box">
          <span className="mass-value">Break it apart, or regroup as you multiply!</span>
          <div style={{ fontSize: '0.85rem', marginTop: 4, color: 'rgba(255,255,255,0.65)' }}>
            Two strategies, one correct answer — pick whichever feels faster
          </div>
        </div>
        ...
        <div className="reflect-badge glass-card">
          ...
          <div>
            ...
            <div className="reflect-badge-sub">Multiplying 4-Digit × 1-Digit and 3-Digit × 2-Digit Numbers</div>
          </div>
        </div>
