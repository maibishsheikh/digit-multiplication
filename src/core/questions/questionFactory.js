...
// preferBoss prioritizes mixedReviewBoss / difficulty-3 questions for Boss
// Battle (falling back to the rest of the pool if there aren't enough).
export function generateModeQuestions(worldId, count, { excludeHard = false, preferBoss = false } = {}) {
  ...
  if (preferBoss) {
    const bossLike = pool.filter((q) => q.type === 'mixedReviewBoss' || q.difficulty === 3);
    if (bossLike.length >= count) {
      pool = bossLike;
    } else {
      const rest = pool.filter((q) => !bossLike.includes(q));
      pool = [...bossLike, ...rest];
    }
  }
