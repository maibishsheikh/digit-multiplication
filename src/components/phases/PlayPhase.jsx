...
export default function PlayPhase({ state, dispatch }) {
  ...
  function advanceQuestion() {
    ...
    if (nextIdx % 10 === 0 && nextIdx <= 100) {
      sounds.levelUp();
      narrate(districtCompleteNarration(district.name));
  ...
  function handleShowHint() {
    ...
    if (hintsShown === 0) {
      setShowHint(1);
      setHintsShown(1);
      narrate(playHint1Narration(question?.hint1));
    } else {
      setShowHint(2);
      setHintsShown(2);
      narrate(playHint2Narration(question?.hint2));
    }
  }
  ...
  if (showMap) {
    ...
    return (
      <div className="play-map-wrap">
        <div className="play-map-card glass-card anim-slide-up">
          <h2 className="play-map-title subheadline">🗺️ Multiplication Worlds Kingdom</h2>
          <p className="body-text" style={{ color: 'var(--color-text-secondary)', textAlign: 'center' }}>
            {isAllDone ? (
              <strong style={{ color: 'var(--gold)' }}>All Multiplication Worlds Complete!</strong>
            ) : (
  ...
  return (
    <div className="play-wrap">
      ...
      <div className="play-top-bar anim-slide-up">
